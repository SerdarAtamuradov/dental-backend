...
SMSru.prototype = {
  sms_send: function (options, callback) {
    ...
    if (
      options.time &&
      options.time < Math.ceil(+new Date() / 1000) + 7 * 60 * 60 * 24
    )
  ...
  get: function (path, params, callback, properties_name) {
    ...
    http
      .get(
        "http://sms.ru/" + path + "?" + querystring.stringify(params),
        function (res) {
          var result = "";
          ...
          res.on("end", function () {
            if (result) {
              ...
              response["description"] =
                response_code[path.split("/")[1]][result[0]];
              result.shift();

              if (typeof properties_name === "string")
                response[properties_name] = [];
              result.forEach(function (id) {
                if (!~id.search(/=/)) {
                  typeof properties_name === "string"
                    ? response[properties_name].push(id)
                    : (response[properties_name.shift()] = id);
                } else {
                  var result = id.split("=");
